...
import { updateCategory } from '../services/AppService';
...
export default function CategoryPage() {
  ...
  const createTemp = () => {
    createTemplateFld(tempCreateObj)
      .then((res) => {
  ...
  const filterPageData = (page, limit) => {
    ...
    filterCategories(reqFilter)
      .then((res) => {
        ...
        const categoryData = dataArray.filter(a => a.id == 2)[0].sub_category.items
        
        if (categoryData.length > 0) {
          setcount(categoryData.length)
